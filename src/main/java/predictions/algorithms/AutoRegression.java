...
public class AutoRegression {
    ...
    public static double[] calculatePredictions(double[] inputseries, double[] arCoefficients, int count, boolean removeMean){
        int length = inputseries.length;
        ...
        double[] estimation = new double[order + count];
        for ( int i = 0; i < order; i++ )
            estimation[i] = w[length - order + i];

        for ( int i = order; i < order + count; i++ ) {
            double est = 0.0;

            for ( int j = 0; j < order; j++ ) {
                est += arCoefficients[j] * estimation[i-(j+1)];
            }

            estimation[i] = est;
        }

        double[] prediction = new double[count];
        for ( int i = 0; i < count; i++ )
            prediction[i] = estimation[i + order];
